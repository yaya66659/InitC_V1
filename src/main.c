...
int main(int argc, char *argv[])
{
 ...
 char messageErreure1[BUFFER_SIZE] = "Nom du projet incorecte ou deja existant\n";
 char messageErreure2[BUFFER_SIZE] = "Erreure saisir O pour oui N pour non(O/N|o/n)\n";


    char makefile = ' ';
        char  nomProjet[BUFFER_SIZE];
    printf("\t\tInitC\n");

    printf("Cree votre projet console en C rapidement .\n");
    ...
    while(1)
    {
            ...
            retScanf = scanf("%255s", nomProjet);
            videBuffer();

            if(retScanf == 1 && !nomProjetExiste(nomProjet) && nomDeProjetEstPermit(nomProjet))
            {
                er1 = false;
    ...
    default:
     ...
     /*************OUVERTURE DEANS VSCODE********* */
    printf("ouverture du projet %s dans VSCode(Si disponible dans variable environement PATH)\n", nomProjet);
   

    VSCode(nomProjet);

    printf("Fin de la créetion sasir ENTER pour quitter");
    getchar();
